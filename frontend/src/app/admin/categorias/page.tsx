  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-8">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#1c1917' }}>Categorías</h1>
      ...
      {categories.length === 0 ? (
        <div className="text-center py-20 rounded-xl border" style={{ backgroundColor: '#ffffff', borderColor: '#e5e0d8' }}>
          <h3 className="text-lg font-medium mb-1" style={{ color: '#44403c' }}>No hay categorías aún</h3>
          <p className="text-sm mb-4" style={{ color: '#78716c' }}>Crea tu primera categoría</p>
          <Link href="/admin/categorias/nueva" className="px-5 py-2.5 font-medium rounded-lg text-sm inline-block" style={{ backgroundColor: '#1c1917', color: '#ffffff' }}>
            + Nueva Categoría
          </Link>
        </div>
      ) : (
        <div>
              ...
              <table className="w-full">
                <thead className="sticky top-0 z-10" style={{ backgroundColor: '#faf7f2' }}>
                  <tr className="border-b" style={{ borderColor: '#e5e0d8' }}>
                    ...
                    <th className="text-left px-4 py-3 text-xs font-medium uppercase tracking-wider" style={{ color: '#78716c' }}>Descripción</th>
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-12 text-center text-sm" style={{ color: '#78716c' }}>
                        {search ? 'No hay categorías que coincidan con la búsqueda' : 'No hay categorías aún'}
                      </td>
                    </tr>
                  ) : filtered.map((category) => (
                    <tr key={category.id} className="transition-colors hover:bg-[#faf7f2]" style={{ backgroundColor: '#ffffff' }}>
                      <td className="px-4 py-4">
                        <span className="text-sm font-medium" style={{ color: '#1c1917' }}>{category.name}</span>
                      </td>
                      <td className="px-4 py-4">
                        <span className="text-sm" style={{ color: '#78716c' }}>{category.description || '—'}</span>
                      </td>
                      <td className="px-4 py-4">
                        <span className="text-sm" style={{ color: '#78716c' }}>{category.order ?? '—'}</span>
                      </td>
                      <td className="px-4 py-4">
                        {category.active ? (
          ...
          <div className="block sm:hidden space-y-3">
            {filtered.length === 0 ? (
              ...
              <div key={category.id} className="rounded-xl border p-4" style={{ backgroundColor: '#ffffff', borderColor: '#e5e0d8' }}>
                ...
                {category.description && (
                  <p className="text-xs mb-2" style={{ color: '#78716c' }}>{category.description}</p>
                )}
                <p className="text-xs mb-3" style={{ color: '#78716c' }}>Orden: {category.order ?? '—'}</p>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="rounded-xl p-6 max-w-sm w-full shadow-xl" style={{ backgroundColor: '#ffffff' }}>
            <h3 className="text-lg font-semibold mb-2" style={{ color: '#1c1917' }}>¿Eliminar categoría?</h3>

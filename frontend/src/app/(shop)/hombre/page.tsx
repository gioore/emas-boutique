...
export default function HombrePage() {
  ...
  const hombreSubcategories: string[] = useMemo(() => {
    const hombreCat = categories.find((c) => c.name?.toLowerCase() === 'hombre');
    if (!hombreCat) return [];
    const subs = (hombreCat as any).subcategories || [];
  ...
  const filtered = useMemo(() => {
    ...
    result = result.filter((p) => p.cat?.name?.toLowerCase() === 'hombre');
    ...
    <div className="min-h-screen" style={{ backgroundColor: BRAND_COLORS.white }}>
      <div className="border-b" style={{ backgroundColor: BRAND_COLORS.background, borderColor: '#e5e0d8' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <h1 className="text-4xl font-bold mt-2" style={{ color: BRAND_COLORS.text }}>Hombre</h1>
          <p className="mt-2" style={{ color: BRAND_COLORS.textMuted }}>Camisas, playeras, pantalones y accesorios importados</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Buscar en Hombre..." value={filters.search}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                ...
                <select value={filters.subcategory} onChange={(e) => updateFilter('subcategory', e.target.value)}
                  ...
                  {hombreSubcategories.map((s) => (<option key={s} value={s}>{s}</option>))}
                </select>
              </div>
              <div>
              ...
              <div>
                ...
                <select value={filters.size} onChange={(e) => updateFilter('size', e.target.value)}
                  ...
                  style={{ borderColor: '#e5e0d8', color: BRAND_COLORS.text }}>
                  <option value="">Todas</option>
                  {['S','M','L','XL','2XL','3XL','Única','39','40','41','42','43','44'].map((s) => (<option key={s} value={s}>{s}</option>))}
                </select>
              </div>
              <div>
                <label className="block text-xs font-medium mb-1.5 uppercase tracking-wider" style={{ color: BRAND_COLORS.textMuted }}>Precio mín</label>
              ...
              <div>
                <label className="block text-xs font-medium mb-1.5 uppercase tracking-wider" style={{ color: BRAND_COLORS.textMuted }}>Precio máx</label>

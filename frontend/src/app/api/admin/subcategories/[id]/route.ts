...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const uniqueSlug = await ensureUniqueSlug(slug, 'subcategories', Number(id));
    await execute(
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    await requireAuth();
    const { id } = await params;
    const refs = await queryOne<{ count: number }>('SELECT COUNT(*) as count FROM products WHERE subcategory_id = $1', [id]);

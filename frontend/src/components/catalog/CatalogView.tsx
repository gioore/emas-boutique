...
export default function CatalogView({ mode, title, subtitle, products, brands, categories, error }: Props) {
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {mode === 'all' && (
        <div>
          ...
          <select id="filter-category" value={filters.category} onChange={(event) => { updateFilter('category', event.target.value); updateFilter('subcategory', ''); }} className="w-full px-3 py-2 border rounded-lg text-sm outline-none" style={{ borderColor: '#e5e5e5', color: BRAND_COLORS.text }}>
      ...
      <div>
        ...
        <select id="filter-subcategory" value={filters.subcategory} onChange={(event) => updateFilter('subcategory', event.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm outline-none" style={{ borderColor: '#e5e5e5', color: BRAND_COLORS.text }}>
          <option value="">Todas</option>
      ...
      <div>
        ...
        <select id="filter-brand" value={filters.brand} onChange={(event) => updateFilter('brand', event.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm outline-none" style={{ borderColor: '#e5e5e5', color: BRAND_COLORS.text }}>
          <option value="">Todas</option>
      ...
      <div>
        ...
        <select id="filter-availability" value={filters.availability} onChange={(event) => updateFilter('availability', event.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm outline-none" style={{ borderColor: '#e5e5e5', color: BRAND_COLORS.text }}>
          <option value="">Todas</option>
      ...
      <div>
        ...
        <select id="filter-size" value={filters.size} onChange={(event) => updateFilter('size', event.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm outline-none" style={{ borderColor: '#e5e5e5', color: BRAND_COLORS.text }}>
          <option value="">Todas</option>
      ...
      <div>
        ...
        <input id="filter-minPrice" type="number" min="0" placeholder="Q0" value={filters.minPrice} onChange={(event) => updateFilter('minPrice', event.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm outline-none" style={{ borderColor: '#e5e5e5', color: BRAND_COLORS.text }} />
      </div>
      <div>
        ...
        <input id="filter-maxPrice" type="number" min="0" placeholder="Q1000" value={filters.maxPrice} onChange={(event) => updateFilter('maxPrice', event.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm outline-none" style={{ borderColor: '#e5e5e5', color: BRAND_COLORS.text }} />
      </div>
      <div className="flex items-end">
        <button onClick={clearFilters} className="px-4 py-2 border rounded-lg text-sm font-medium transition-colors w-full" style={{ borderColor: '#e5e5e5', color: BRAND_COLORS.textMuted }}>
          Limpiar filtros
        </button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: mode === 'all' ? BRAND_COLORS.background : BRAND_COLORS.white }}>
      <div className="border-b" style={{ backgroundColor: mode === 'all' ? BRAND_COLORS.white : BRAND_COLORS.background, borderColor: '#e5e5e5' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div ref={sortBarRef} className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="flex-1 relative">
            ...
            <input
              ...
              style={{ borderColor: '#e5e5e5', backgroundColor: BRAND_COLORS.white, color: BRAND_COLORS.text }}
            />
          </div>
          <div className="flex gap-3">
            <select value={filters.sort} onChange={(event) => updateFilter('sort', event.target.value)} className="px-4 py-3 border rounded-xl outline-none text-sm" style={{ borderColor: '#e5e5e5', backgroundColor: BRAND_COLORS.white, color: BRAND_COLORS.text }}>
        ...
        {sticky && (
          <div className="sticky top-16 z-30 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 py-3 border-b shadow-sm" style={{ backgroundColor: 'rgba(250,247,242,0.95)', backdropFilter: 'blur-sm', borderColor: '#e5e5e5' }}>
            <div className="max-w-7xl mx-auto flex items-center justify-between">
              ...
              <div className="flex gap-3">
                <select value={filters.sort} onChange={(event) => updateFilter('sort', event.target.value)} className="px-3 py-2 border rounded-lg outline-none text-xs" style={{ borderColor: '#e5e5e5', backgroundColor: BRAND_COLORS.white, color: BRAND_COLORS.text }}>
                  <option value="newest">Mas recientes</option>
                  <option value="price-asc">Menor precio</option>
                  <option value="price-desc">Mayor precio</option>
                  <option value="name-asc">A-Z</option>
                ...
                <button onClick={() => setShowFilters(true)} className="px-3 py-2 border rounded-lg text-xs font-medium" style={{ borderColor: '#e5e5e5', color: BRAND_COLORS.text }} aria-label="Abrir filtros">
        ...
        <div className="hidden lg:block">
          {showFilters && (
            <div className="mb-8 p-6 rounded-xl border animate-scale-in" style={{ backgroundColor: BRAND_COLORS.white, borderColor: '#e5e5e5' }}>
        ...
        {filtered.length > 0 ? (
          <>
            <div role="list" className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 lg:gap-6">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-10">
                <button onClick={() => goToPage(safePage - 1)} disabled={safePage <= 1}
                  ...
                  style={{ backgroundColor: '#f5f5f5', color: '#44403c' }}>
                  Anterior
                </button>
